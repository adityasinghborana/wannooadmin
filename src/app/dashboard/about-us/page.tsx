...
import { GetAllImages, getAboutUsDetails, updateAboutUsDetails } from "@/lib/services";
...
const Home: React.FC = () => {
  const [aboutUsData, setAboutUsPageData] = useState<HomePageData>({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getAboutUsDetails();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateAboutUsDetails(aboutUsData);
      ...
      >
        ...
        >
          <form onSubmit={handleSubmit} className="grid grid-cols-3 gap-4">
            ...
            {Object.keys(aboutUsData).map((key) => {
              ...
              return (
                <div className="mb-4" key={key}>
                  ...
                  <input
                    type="text"
                    name={key}
                    value={aboutUsData[key as keyof HomePageData]}
